...
__constant__ uint32_t T0[256];
__constant__ uint32_t T1[256];
__constant__ uint32_t T2[256];
__constant__ uint32_t T3[256];
...
__global__ void aes256_kernel(byte* data, size_t numBlocks){
...
int main(int argc, char** argv) {
    ...
    gpuErrchk(cudaMemcpyToSymbol(T0, h_T0, 256*sizeof(uint32_t)));
    gpuErrchk(cudaMemcpyToSymbol(T1, h_T1, 256*sizeof(uint32_t)));
    gpuErrchk(cudaMemcpyToSymbol(T2, h_T2, 256*sizeof(uint32_t)));
    gpuErrchk(cudaMemcpyToSymbol(T3, h_T3, 256*sizeof(uint32_t)));

    aes256_kernel<<<blocks, threads>>>(d_data, numBlocks);
