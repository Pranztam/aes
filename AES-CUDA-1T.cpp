#include <iostream>
#include <random>
#include <cstring>
#include <openssl/evp.h>
#include <fstream>
#include "AES.hpp"

#define AES256_ROUNDS 14
#define EXPANDED_KEY_SIZE 240

constexpr size_t BLOCK_SIZE = 16;

using byte = unsigned char;

inline void gpuAssert(cudaError_t code, const char *file, int line)
{
    if (code != cudaSuccess) {
        std::cerr << "GPUassert code: " << cudaGetErrorString(code) << ", file: " << file << ", line: " << line << std::endl;
        exit(code);
    }
}

#define gpuErrchk(ans) { gpuAssert((ans), __FILE__, __LINE__); }

inline uint64_t current_time_nsecs()
{
    struct timespec t;
    clock_gettime(CLOCK_REALTIME, &t);
    return (t.tv_sec)*1000000000L + t.tv_nsec;
}

//setting up creation of uint32_t data (T-tables)
cudaTextureObject_t createTexture_uint32(uint32_t* d_ptr, size_t count) {
    cudaResourceDesc resDesc;
    memset(&resDesc, 0, sizeof(resDesc));
    resDesc.resType = cudaResourceTypeLinear;
    resDesc.res.linear.devPtr = d_ptr;
    resDesc.res.linear.desc = cudaCreateChannelDesc<uint32_t>();
    resDesc.res.linear.sizeInBytes = count * sizeof(uint32_t);

    cudaTextureDesc texDesc;
    memset(&texDesc, 0, sizeof(texDesc));
    texDesc.readMode = cudaReadModeElementType;

    cudaTextureObject_t texObj;
    gpuErrchk(cudaCreateTextureObject(&texObj, &resDesc, &texDesc, NULL));
    return texObj;
}

//and of byte data (sbox, roundkeys, nonce)
cudaTextureObject_t createTexture_byte(byte* d_ptr, size_t count) {
    cudaResourceDesc resDesc;
    memset(&resDesc, 0, sizeof(resDesc));
    resDesc.resType = cudaResourceTypeLinear;
    resDesc.res.linear.devPtr = d_ptr;
    resDesc.res.linear.desc = cudaCreateChannelDesc<uint8_t>();
    resDesc.res.linear.sizeInBytes = count * sizeof(byte);

    cudaTextureDesc texDesc;
    memset(&texDesc, 0, sizeof(texDesc));
    texDesc.readMode = cudaReadModeElementType;

    cudaTextureObject_t texObj;
    gpuErrchk(cudaCreateTextureObject(&texObj, &resDesc, &texDesc, NULL));
    return texObj;
}

byte h_sbox[256] = {
        
};

//gmul is an implementation of multiplication in GF(2^8) finite field, needed to compute the t-tables.
//more specifically, the only values of b are either 2 or 3, because in the mixcolumns matrix we only have '1','2','3' multipliers, as it can be seen from the the AES NIST standard paper.
//gmul performs a multiplication in the GF(2^8) finite field. This particular implementation is called "Russian peasant" -> https://en.wikipedia.org/wiki/Finite_field_arithmetic#C_programming_example
byte gmul(byte a, byte b) {
    byte p = 0;
    for (int i = 0; i < 8; i++) {
        if (b & 1) p ^= a;
        if (a & 0x80)
            a = (a << 1) ^ 0x11b;
        else
            a <<= 1;
        
        b >>= 1;
    }
    return p;
}

//rotate left of a 32-bit word by 1 byte
uint32_t rotate_word(uint32_t x) {
    return (x >> 8) | (x << 24);
}

//creation of the 4-tables
void t_tables(uint32_t* T0, uint32_t* T1, uint32_t* T2, uint32_t* T3) {

    for (int x = 0; x < 256; x++) {

        //the base value of the computation of the t-table is taken from the sbox so that it can substitute the SubBytes operation in the main rounds
        byte s = h_sbox[x];

        
        byte s2 = gmul(s, 2);
        byte s3 = gmul(s, 3);

        //creating each entry of t-tables(32 bit words) -> with this structure we can see the coefficients of the first column of the mixcolumn matrix, which are 2,1,1,3
        //the other 3 tables are obtained by just rotating the row left of 1 byte and represent the other 3 rows of the mix column matrix coefficients.
        uint32_t t = (s2 << 24) | (s  << 16) | (s  << 8) | (s3);

        T0[x] = t;
        T1[x] = rotate_word(t);
        T2[x] = rotate_word(T1[x]);
        T3[x] = rotate_word(T2[x]);
    }
}

//last round (14th) of the 256 bit encryption. It requires a separate computation because it does not perform a mixcolumn operation
//for simplicity of writing, it has been separated from the main rounds computation where we can use bytes instead of 32 bit words as units.
__device__ void final_round(uint32_t* state, cudaTextureObject_t tex_roundKeys, cudaTextureObject_t tex_sbox, int round_offset) {
    byte s[16];

    //we don't need the state represented as 4 32-bit words, so we extract each byte
    for (int i = 0; i < 4; i++) {
        s[i*4+0] = (state[i] >> 24) & 0xff;
        s[i*4+1] = (state[i] >> 16) & 0xff;
        s[i*4+2] = (state[i] >> 8) & 0xff;
        s[i*4+3] = (state[i]) & 0xff;
    }

    byte temp_state[16];

    //here we perform a SubBytes and ShiftRows operation simultaneously. Instead of assigning the value of the shifted row, we use corresponding sbox value.
    //0  4  8  12        0  4  8  12
    //1  5  9  13   ---> 5  9  13 1
    //2  6  10 14        10 14 2  6
    //3  7  11 15        15 3  7  11
    temp_state[0]  = tex1Dfetch<uint8_t>(tex_sbox, s[0]);
    temp_state[1]  = tex1Dfetch<uint8_t>(tex_sbox, s[5]);
    temp_state[2]  = tex1Dfetch<uint8_t>(tex_sbox, s[10]);
    temp_state[3]  = tex1Dfetch<uint8_t>(tex_sbox, s[15]);

    temp_state[4]  = tex1Dfetch<uint8_t>(tex_sbox, s[4]);
    temp_state[5]  = tex1Dfetch<uint8_t>(tex_sbox, s[9]);
    temp_state[6]  = tex1Dfetch<uint8_t>(tex_sbox, s[14]);
    temp_state[7]  = tex1Dfetch<uint8_t>(tex_sbox, s[3]);

    temp_state[8]  = tex1Dfetch<uint8_t>(tex_sbox, s[8]);
    temp_state[9]  = tex1Dfetch<uint8_t>(tex_sbox, s[13]);
    temp_state[10] = tex1Dfetch<uint8_t>(tex_sbox, s[2]);
    temp_state[11] = tex1Dfetch<uint8_t>(tex_sbox, s[7]);

    temp_state[12] = tex1Dfetch<uint8_t>(tex_sbox, s[12]);
    temp_state[13] = tex1Dfetch<uint8_t>(tex_sbox, s[1]);
    temp_state[14] = tex1Dfetch<uint8_t>(tex_sbox, s[6]);
    temp_state[15] = tex1Dfetch<uint8_t>(tex_sbox, s[11]);

    //adding round key
    for (int i = 0; i < 16; i++)
        temp_state[i] ^= tex1Dfetch<uint8_t>(tex_roundKeys, round_offset + i);

    //recreating the 4 32-bit words comprising the state
    for (int i = 0; i < 4; i++)
        state[i] = (temp_state[i*4+0] << 24) | (temp_state[i*4+1] << 16) | (temp_state[i*4+2] << 8) | (temp_state[i*4+3]);
}

//main encrypting function (rounds). Note that even though the state of AES is a column oriented data structure, we can still reason in a row-like manner:
//data input: b0 b1 b2 b3 b4 b5 b6 b7 b8 b9 b10 b11 b12 b13 b14 b15 -> b0 b1 b2 b3 create a column, but in our case we receive them sequentially and therefore can treat them as a row.
__global__ void aes256_kernel(byte* data, cudaTextureObject_t tex_roundKeys, cudaTextureObject_t tex_nonce, cudaTextureObject_t tex_sbox, size_t numBlocks, cudaTextureObject_t tex_T0, cudaTextureObject_t tex_T1, cudaTextureObject_t tex_T2, cudaTextureObject_t tex_T3){
// __global__ void aes256_kernel(byte* data, byte* roundKeys, byte* nonce, byte* d_sbox, size_t numBlocks, uint32_t* T0, uint32_t* T1, uint32_t* T2, uint32_t* T3){
    size_t idx = blockIdx.x * blockDim.x + threadIdx.x;

    if (idx >= numBlocks) return;

    //creating the state that will be encrypted. It contains nonce || ctr. In our case idx is a perfect ctr
    uint32_t state[4];  
    for(int i = 0; i < 3; i++)
            state[i] = (tex1Dfetch<uint8_t>(tex_nonce, i*4+0) << 24) | (tex1Dfetch<uint8_t>(tex_nonce, i*4+1) << 16) | (tex1Dfetch<uint8_t>(tex_nonce, i*4+2) << 8) | (tex1Dfetch<uint8_t>(tex_nonce, i*4+3));
        // state[i] = (nonce[i*4+0] << 24) | (nonce[i*4+1] << 16) | (nonce[i*4+2] << 8) | (nonce[i*4+3]);
    state[3] = static_cast<uint32_t>(idx); 
    

    //before the regular rounds, we perform the first add round key after we compact the expanded key in 32-bit words (in regular rounds, the add round key operation does the same thing
    //except in final round, whose unit of work is bytes instead of 32-bit words. See final_round() above)
    for (int i = 0; i < 4; i++) {
        // uint32_t comp_k = (roundKeys[i*4+0] << 24) | (roundKeys[i*4+1] << 16) | (roundKeys[i*4+2] << 8) | (roundKeys[i*4+3]);
        uint32_t comp_k = (tex1Dfetch<uint8_t>(tex_roundKeys, i*4+0) << 24) | (tex1Dfetch<uint8_t>(tex_roundKeys, i*4+1) << 16) | (tex1Dfetch<uint8_t>(tex_roundKeys, i*4+2) << 8) | (tex1Dfetch<uint8_t>(tex_roundKeys, i*4+3));
        state[i] ^= comp_k;
    }
    
    //main rounds, which perform SubBytes, ShiftRows and MixColumns thanks to the t-tables, whose entries are 32-bits words each and contain 256 elements.
	//more precisely the SubBytes and MixColumns are integrated in the t-tables creation, as we've seen above, whilst now we are performing a
	//ShiftRows operation by manipulating the state array indexes in the assignments
	uint32_t t_table[4];
    for (int round = 1; round < AES256_ROUNDS; round++) {
        t_table[0] = tex1Dfetch<uint32_t>(tex_T0, (state[0] >> 24) & 0xff) ^ tex1Dfetch<uint32_t>(tex_T1, (state[1] >> 16) & 0xff) ^ tex1Dfetch<uint32_t>(tex_T2, (state[2] >>  8) & 0xff) ^ tex1Dfetch<uint32_t>(tex_T3, (state[3]) & 0xff);
        t_table[1] = tex1Dfetch<uint32_t>(tex_T0, (state[1] >> 24) & 0xff) ^ tex1Dfetch<uint32_t>(tex_T1, (state[2] >> 16) & 0xff) ^ tex1Dfetch<uint32_t>(tex_T2, (state[3] >>  8) & 0xff) ^ tex1Dfetch<uint32_t>(tex_T3, (state[0]) & 0xff);
        t_table[2] = tex1Dfetch<uint32_t>(tex_T0, (state[2] >> 24) & 0xff) ^ tex1Dfetch<uint32_t>(tex_T1, (state[3] >> 16) & 0xff) ^ tex1Dfetch<uint32_t>(tex_T2, (state[0] >>  8) & 0xff) ^ tex1Dfetch<uint32_t>(tex_T3, (state[1]) & 0xff);
        t_table[3] = tex1Dfetch<uint32_t>(tex_T0, (state[3] >> 24) & 0xff) ^ tex1Dfetch<uint32_t>(tex_T1, (state[0] >> 16) & 0xff) ^ tex1Dfetch<uint32_t>(tex_T2, (state[1] >>  8) & 0xff) ^ tex1Dfetch<uint32_t>(tex_T3, (state[2]) & 0xff);
        // t_table[0] = T0[(state[0] >> 24) & 0xff] ^ T1[(state[1] >> 16) & 0xff] ^ T2[(state[2] >> 8) & 0xff] ^ T3[(state[3]) & 0xff];
        // t_table[1] = T0[(state[1] >> 24) & 0xff] ^ T1[(state[2] >> 16) & 0xff] ^ T2[(state[3] >> 8) & 0xff] ^ T3[(state[0]) & 0xff];
        // t_table[2] = T0[(state[2] >> 24) & 0xff] ^ T1[(state[3] >> 16) & 0xff] ^ T2[(state[0] >> 8) & 0xff] ^ T3[(state[1]) & 0xff];
        // t_table[3] = T0[(state[3] >> 24) & 0xff] ^ T1[(state[0] >> 16) & 0xff] ^ T2[(state[1] >> 8) & 0xff] ^ T3[(state[2]) & 0xff];

        //add round key
        for (int i = 0; i < 4; i++) {
            uint32_t comp_k = (tex1Dfetch<uint8_t>(tex_roundKeys, round*16 + i*4+0) << 24) | (tex1Dfetch<uint8_t>(tex_roundKeys, round*16 + i*4+1) << 16) | (tex1Dfetch<uint8_t>(tex_roundKeys, round*16 + i*4+2) << 8) | (tex1Dfetch<uint8_t>(tex_roundKeys, round*16 + i*4+3));
            // uint32_t comp_k = (roundKeys[round*16 + i*4+0] << 24) | (roundKeys[round*16 + i*4+1] << 16) | (roundKeys[round*16 + i*4+2] << 8) | (roundKeys[round*16 + i*4+3]);
			t_table[i] ^= comp_k;
        }

        //finally we recreate the state from the t-tables
        state[0] = t_table[0];
        state[1] = t_table[1];
        state[2] = t_table[2];
        state[3] = t_table[3];

    }

    //perform final round (see final_round()) above
    final_round(state, tex_roundKeys, tex_sbox, AES256_ROUNDS * 16);

    //store back the state in the data array by decomposing it in bytes again
    //positioning the thread on the correct data index based on its identifier
    data = data + (idx * 16);
    for (int i = 0; i < 4; i++) {
        data[i*4+0] ^= (state[i] >> 24) & 0xff;
        data[i*4+1] ^= (state[i] >> 16) & 0xff;
        data[i*4+2] ^= (state[i] >> 8) & 0xff;
        data[i*4+3] ^= state[i] & 0xff;
    }
    
}

int main(int argc, char** argv) {
    if(argc < 2){
        std::cerr<<"Usage: ./AES-CUDA <size of text to generate in MB>"<<std::endl;
        return -1;
    }

    if(atoi(argv[1]) == 0 || atoi(argv[1]) > 1024){
        std::cerr<<"Size must be between 1 and 1024"<<std::endl;
        return -1;
    }

    size_t SIZE_MB = atoi(argv[1])*1024*1024;

    std::random_device rd;
    std::mt19937 gen(rd());
    std::uniform_int_distribution<int> dist(0, 255);

    //plaintext generation
    byte* h_data = (byte*)malloc(SIZE_MB);
    for (size_t i = 0; i < SIZE_MB; i++)
        h_data[i] = static_cast<byte>(dist(gen));

    //save original plain text in reference before copying the encrypted data back in h_data
    byte* reference = (byte*)malloc(SIZE_MB);
    memcpy(reference, h_data, SIZE_MB);

    size_t numBlocks = SIZE_MB / 16;

    //key generation
    byte key[32];
    for (int i = 0; i < 32; ++i)
        key[i] = static_cast<byte>(dist(gen));

    //nonce generation
    byte h_nonce[12];
    for (int i = 0; i < 12; i++)
        h_nonce[i] = static_cast<byte>(dist(gen));

	//here we call the constructor of the class cipher, included in AES.hpp, which transforms the key passed as a parameter
	//into an expanded key of 240 bits
    Cipher::Aes<256> aes(key);

    //t-tables creation
	uint32_t h_T0[256], h_T1[256], h_T2[256], h_T3[256];
	t_tables(h_T0, h_T1, h_T2, h_T3);

	uint32_t *d_T0, *d_T1, *d_T2, *d_T3;

    int threads = 256;
    int blocks = (numBlocks + threads - 1) / threads;

    //d_data = device data, d_keys = expanded key for aes256 (240 bits)
    byte *d_data, *d_keys, *d_nonce, *d_sbox;


    uint64_t start_time = current_time_nsecs();


    gpuErrchk(cudaMalloc(&d_data, SIZE_MB));
    gpuErrchk(cudaMalloc(&d_nonce, 12));
    gpuErrchk(cudaMalloc(&d_keys, EXPANDED_KEY_SIZE));
    gpuErrchk(cudaMalloc(&d_sbox, 256));
    gpuErrchk(cudaMemcpy(d_data, h_data, SIZE_MB, cudaMemcpyHostToDevice));
    gpuErrchk(cudaMemcpy(d_nonce, h_nonce, 12, cudaMemcpyHostToDevice));
    gpuErrchk(cudaMemcpy(d_keys, aes.getRoundKeys(), EXPANDED_KEY_SIZE, cudaMemcpyHostToDevice));
    gpuErrchk(cudaMemcpy(d_sbox, h_sbox, 256, cudaMemcpyHostToDevice));

    gpuErrchk(cudaMalloc(&d_T0, 256 * sizeof(uint32_t)));
	gpuErrchk(cudaMalloc(&d_T1, 256 * sizeof(uint32_t)));
	gpuErrchk(cudaMalloc(&d_T2, 256 * sizeof(uint32_t)));
	gpuErrchk(cudaMalloc(&d_T3, 256 * sizeof(uint32_t)));

	gpuErrchk(cudaMemcpy(d_T0, h_T0, 256 * sizeof(uint32_t), cudaMemcpyHostToDevice));
	gpuErrchk(cudaMemcpy(d_T1, h_T1, 256 * sizeof(uint32_t), cudaMemcpyHostToDevice));
	gpuErrchk(cudaMemcpy(d_T2, h_T2, 256 * sizeof(uint32_t), cudaMemcpyHostToDevice));
	gpuErrchk(cudaMemcpy(d_T3, h_T3, 256 * sizeof(uint32_t), cudaMemcpyHostToDevice));

    cudaTextureObject_t tex_roundKeys = createTexture_byte(d_keys, EXPANDED_KEY_SIZE);
    cudaTextureObject_t tex_nonce = createTexture_byte(d_nonce, 12);
    cudaTextureObject_t tex_sbox = createTexture_byte(d_sbox, 256);
    cudaTextureObject_t tex_T0 = createTexture_uint32(d_T0, 256);
    cudaTextureObject_t tex_T1 = createTexture_uint32(d_T1, 256);
    cudaTextureObject_t tex_T2 = createTexture_uint32(d_T2, 256);
    cudaTextureObject_t tex_T3 = createTexture_uint32(d_T3, 256);

    aes256_kernel<<<blocks, threads>>>(d_data, tex_roundKeys, tex_nonce, tex_sbox, numBlocks, tex_T0, tex_T1, tex_T2, tex_T3);
    gpuErrchk(cudaDeviceSynchronize());
    gpuErrchk(cudaMemcpy(h_data, d_data, SIZE_MB, cudaMemcpyDeviceToHost));

    uint64_t end_time = current_time_nsecs();
    std::cout<<"elapsed time: "<< end_time - start_time<<std::endl;

    //correctness check comparing the original data array and a reference array encrypted using an OpenSSL library function
    byte iv[16];
    memcpy(iv, h_nonce, 12);
    iv[12] = iv[13] = iv[14] = iv[15] = 0;

    EVP_CIPHER_CTX* ctx = EVP_CIPHER_CTX_new();
    EVP_EncryptInit_ex(ctx, EVP_aes_256_ctr(), NULL, key, iv);

    int outlen;
    EVP_EncryptUpdate(ctx, reference, &outlen, reference, SIZE_MB);
    EVP_CIPHER_CTX_free(ctx);

    if (memcmp(h_data, reference, SIZE_MB) == 0)
        std::cout << "Encryption correct"<<std::endl;
    else
        std::cout << "Error in the encryption"<<std::endl;
    
    gpuErrchk(cudaFree(d_data));
    gpuErrchk(cudaFree(d_keys));
    gpuErrchk(cudaFree(d_nonce));
    gpuErrchk(cudaFree(d_sbox));
    gpuErrchk(cudaFree(d_T0));
    gpuErrchk(cudaFree(d_T1));
    gpuErrchk(cudaFree(d_T2));
    gpuErrchk(cudaFree(d_T3));

    cudaDestroyTextureObject(tex_T0);
    cudaDestroyTextureObject(tex_T1);
    cudaDestroyTextureObject(tex_T2);
    cudaDestroyTextureObject(tex_T3);
    cudaDestroyTextureObject(tex_sbox);
    cudaDestroyTextureObject(tex_roundKeys);
    cudaDestroyTextureObject(tex_nonce);

    free(h_data);
    free(reference);

    return 0;
}