...
__global__ void aes256_kernel(byte* data, byte* d_roundKeys, byte* d_nonce, size_t numBlocks, uint32_t* d_T0, uint32_t* d_T1, uint32_t* d_T2, uint32_t* d_T3){
    size_t idx = blockIdx.x * blockDim.x + threadIdx.x;

    if (idx >= numBlocks) return;

    __shared__ byte s_sbox[256];
    __shared__ byte roundKeys[EXPANDED_KEY_SIZE];
    __shared__ byte nonce[12];
    ...
    //populating shared memory data structures
    ...
    if(threadIdx.x < EXPANDED_KEY_SIZE)
        roundKeys[threadIdx.x] = d_roundKeys[threadIdx.x];
    if(threadIdx.x < 12)
        nonce[threadIdx.x] = d_nonce[threadIdx.x];
